...
def flydsl_hgemm(
    ...
    *,
    ...
    When ``prezero=True`` (``hgemm`` and ``small_m`` families) the split-K kernel
    skips the in-kernel zero-init / signal handshake and atomic-adds straight into a
    pre-zeroed ``C``. A supplied ``out`` MUST already be zeroed; if ``out is None`` it
    is allocated with ``torch.zeros``.
    """
    ...
    if out is None:
        alloc = torch.zeros if prezero else torch.empty
